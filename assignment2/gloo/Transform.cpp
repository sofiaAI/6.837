...
glm::mat4 Transform::GetLocalToAncestorMatrix(SceneNode* ancestor) const {
  // calculate local-to-ancestor matrix
  // optionally implement this method which can become useful in SSD.

  if (node_.GetParentPtr() == ancestor) {
    return local_transform_mat_;
  }
  return node_.GetParentPtr()->GetTransform().GetLocalToWorldMatrix() * local_transform_mat_;
}
