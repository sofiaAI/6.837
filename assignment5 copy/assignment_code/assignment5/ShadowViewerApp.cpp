...
void ShadowViewerApp::SetupScene() {
  ...
  // EXTRA CREDIT ADD SPOT LIGHT
  if(spot_light_enabled){
      auto spot_light = std::make_shared<SpotLight>();
      spot_light->SetSpotDirection(glm::vec3(0.0, -1.0, 0.0));
      spot_light->SetCutoffAngle(-0.80f);
      spot_light->SetColor(glm::vec3(0.1, 0.1, 0.1));

      auto spot_node = make_unique<SceneNode>();
      spot_node->CreateComponent<LightComponent>(spot_light);
      spot_node->GetTransform().SetPosition(glm::vec3(0.0, 3.5, 0.0));
      root.AddChild(std::move(spot_node));
  }
  ...
  for(int i = 0; i < mesh_data.groups.size(); i++){
      ...
      //give it a shading, rendering, and material component
      scene_node->CreateComponent<ShadingComponent>(shader);
      scene_node->CreateComponent<RenderingComponent>(vertex_obj);
      MeshGroup group = mesh_data.groups[i];
      //limit the rendering range
      scene_node->GetComponentPtr<RenderingComponent>()->SetDrawRange(group.start_face_index, group.num_indices);
      scene_node->CreateComponent<MaterialComponent>(group.material);
      //VertexObject is split into a few segments of indices
