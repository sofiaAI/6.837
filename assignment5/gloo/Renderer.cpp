...
Renderer::Renderer(Application& application) : application_(application) {
  ...
  // initialize the shadow_depth_tex_ object with resolution 4096×4096
  ...
  framebuffer = make_unique<Framebuffer>();
  framebuffer->AssociateTexture(*shadow_depth_tex_, GL_DEPTH_ATTACHMENT);

  //INIT SHADER
  shadow_shader = make_unique<ShadowShader>();
...
void Renderer::RenderScene(const Scene& scene) const {
  ...
  for (size_t light_id = 0; light_id < light_ptrs.size(); light_id++) {
    ...
    if(light_ptrs[light_id]->CanCastShadow()){
      BindGuard frame_buffer_bg(framebuffer.get());
      BindGuard shadow_shader_bg(shadow_shader.get());
      world_to_light_ndc_matrix = Renderer::RenderShadow(light_ptrs[light_id], rendering_info);
    }
...
glm::mat4 Renderer::RenderShadow(LightComponent* light_component, RenderingInfo rendering_info) const{
  ...
  glm::mat4 view_matrix = light_component->GetNodePtr()->GetTransform().GetLocalToWorldMatrix();
  glm::mat4 world_to_light_ndc_matrix = kLightProjection*glm::inverse(view_matrix);
  ...
  for (const auto& pr : rendering_info) {
    ...
    shadow_shader->SetMatrix(world_to_light_ndc_matrix);
  ...
  return world_to_light_ndc_matrix;
}
