...
#include "gloo/lights/SpotLight.hpp"
...
void PhongShader::SetTargetNode(const SceneNode& node,
  ...
  std::shared_ptr<Texture> ambient_tex = material_ptr->GetAmbientTexture();
  std::shared_ptr<Texture> diffuse_tex = material_ptr->GetDiffuseTexture();
  std::shared_ptr<Texture> specular_tex = material_ptr->GetSpecularTexture();

  if(ambient_tex != nullptr){ ambient_tex->BindToUnit(0); }
  if(diffuse_tex != nullptr){ diffuse_tex->BindToUnit(1); }
  if(specular_tex != nullptr){ specular_tex->BindToUnit(2); }

  SetUniform("ambient_sampler", 0);
  SetUniform("diffuse_sampler", 1);
  SetUniform("specular_sampler", 2);

  SetUniform("normal_sampler", 5);
  SetUniform("noise_sampler", 6);
  SetUniform("position_sampler", 7);
...
void PhongShader::SetLightSource(const LightComponent& component) const {
  ...
  SetUniform("spot_light.enabled", false);
  ...
  }else if (light_ptr->GetType() == LightType::Spot){
    auto spot_light_ptr = static_cast<SpotLight*>(light_ptr);
    SetUniform("spot_light.enabled", true);
    SetUniform("spot_light.position",
               component.GetNodePtr()->GetTransform().GetPosition());
    SetUniform("spot_light.spot_direction",
               spot_light_ptr->GetSpotDirection());
    SetUniform("spot_light.cutoff_angle",
               spot_light_ptr->GetCutoffAngle());
    SetUniform("spot_light.color",
               spot_light_ptr->GetColor());
...
void PhongShader::SetShadowMapping(const Texture& shadow_texture, const glm::mat4& world_to_light_ndc_matrix) const {
